import api from './api';
import { CreateEventPayload, UpdateEventPayload, Event, EventTypeOption } from '../types/event';
...
export const eventService = {
  ...
  async updateEvent(eventId: number, payload: UpdateEventPayload): Promise<Event> {
  ...
  async acceptEvent(eventId: number): Promise<{ message: string }> {
    const response = await api.post<{ message: string }>(`/events/${eventId}/accept`);
    return response.data;
  },

  async declineEvent(eventId: number): Promise<{ message: string }> {
    const response = await api.post<{ message: string }>(`/events/${eventId}/decline`);
    return response.data;
  },
